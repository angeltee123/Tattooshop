...
checkout_form.addEventListener('change', function(){
  ...
  value = payment_method.value.toString().trim();
  message = payment_method_err.lastChild;
  if(value.length == 0 || ["Cash", "Check"].includes(value) == false){
    payment_method.classList.remove('is-valid');
    payment_method.classList.add('is-invalid');

    message.innerText = (value.length == 0) ? "Payment method is required." : "Payment method must be either Cash or Check.";

    payment_method_err.classList.replace('d-none', 'd-flex');
    errors.push(message.innerText || message.innerHTML);
  } else {
    payment_method.classList.remove('is-invalid');
    payment_method.classList.add('is-valid');
